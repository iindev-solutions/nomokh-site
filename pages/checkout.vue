...
<template>
  <section class="container-page py-12">
    <h1 class="text-5xl font-semibold">Оформление заказа</h1>
    <CheckoutStepper class="mt-8" :active="2" />
    <div v-if="result" class="card-dark mt-8 p-8">
      <p class="text-2xl font-semibold">Заказ {{ result.order.orderNumber }} создан</p>
    ...
    <form v-else class="mt-8 grid gap-8 lg:grid-cols-[1fr_22rem]" @submit.prevent="submit">
      <div class="card-dark space-y-5 p-6">
        ...
        <p class="text-xs leading-5 text-steel">Нажимая кнопку, вы соглашаетесь с обработкой персональных данных.</p>
      ...
      <aside class="card-dark h-fit p-6">
        ...
        <p class="mt-3 text-4xl font-semibold text-forge">{{ cart.totalAmount.toLocaleString('ru-RU') }} ₽</p>
