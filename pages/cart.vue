      ...
      <div class="space-y-4">
        <div v-for="item in cart.items" :key="item.id" class="card-dark flex gap-4 p-4">
          ...
          <div class="flex flex-1 flex-col justify-between gap-4 sm:flex-row">
            <div><NuxtLink :to="`/product/${item.slug}`" class="text-xl font-semibold hover:text-forge">{{ item.name }}</NuxtLink><p class="mt-2 text-steel">{{ item.price.toLocaleString('ru-RU') }} ₽</p></div>
